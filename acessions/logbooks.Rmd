...
```{r}
dpe2022 = read.csv(paste0(dados,'logbooks/dpe2022.csv'))
# load(paste0(dados,'logbooks/dpe2021_correct.RData'))
# load("C:/PNAB/fleet_register/fleet.Rdata") # casa
load("C:/dados_pnab/fleet_register/fleet.Rdata")
...
df = df %>%
  mutate(DATAHORA_INICIO = as.POSIXct(DATAHORA_INICIO, format = '%Y-%m-%d %H:%M:%S'),
         DATAHORA_FIM = as.POSIXct(DATAHORA_FIM, format = '%Y-%m-%d %H:%M:%S'),
         DEPARTURE_DATAHORA = as.POSIXct(DEPARTURE_DATAHORA, format = '%Y-%m-%d %H:%M:%S'),
         RETURN_DATAHORA = as.POSIXct(RETURN_DATAHORA, format = '%Y-%m-%d %H:%M:%S'),
...
temp[is.na(temp$Power.of.main.engine),] %>% View
